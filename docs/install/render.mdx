...
| Variable | Value |
| --- | --- |
| `FILESLINK_API_BASE_URL` | Defaults to `https://tcloud-2f7u.onrender.com`; override only if the hosted service changes |
...
At startup, the container discovers the newest FilesLink manifest, downloads and hash-verifies every archive chunk, restores the archive into a fresh staging directory, activates the recorded assets, and runs `openclaw doctor` before starting the Gateway. A background loop creates verified backups using `openclaw backup create --output <directory> --verify`, splits the archive into 45 MiB pieces, and uploads them one at a time. No FilesLink API key or channel ID is required; the hosted service owns the Telegram storage configuration.

The configured endpoint is the hosted FilesLink Rust service, which implements `/api/files/upload?filename=...`, `/api/files`, and `/files/{unique_id}`. The service must remain deployed and its Telegram bot must retain access to its configured storage channel before OpenClaw can restore or upload state.
